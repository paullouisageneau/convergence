...
/*
Adapted from the public domain implementation by the Keccak Team, namely,
 ...
 *                         Some examples:
 *                          - If no bits are to be appended, then @a delimitedSuffix must be
 *                            0x01.
...
void Keccak(unsigned int rate, unsigned int capacity, const uint8_t *input, size_t inputByteLen,
	...
	std::fill(state, state + 200, 0);

	/* === Absorb all the input blocks === */
	while (inputByteLen > 0) {
		blockSize = std::min(inputByteLen, size_t(rateInBytes));
	...
	/* Absorb the last few bits and add the first bit of padding (which coincides with the delimiter
	 * in delimitedSuffix) */
	state[blockSize] ^= delimitedSuffix;
	/* If the first bit of padding is at position rate-1, we need a whole new block for the second
	 * bit of padding */
	...
	while (outputByteLen > 0) {
		blockSize = std::min(outputByteLen, size_t(rateInBytes));
		std::copy(state, state + blockSize, output);
		output += blockSize;
...
void Shake128(const binary &input, binary &output, size_t len) {
	output.resize(len);
	Keccak(1344, 256, u8(input.data()), input.size(), 0x1F, u8(output.data()), output.size());
}
...
void Shake256(const binary &input, binary &output, size_t len) {
	output.resize(len);
	Keccak(1088, 512, u8(input.data()), input.size(), 0x1F, u8(output.data()), output.size());
}
...
void Sha3_224(const binary &input, binary &output) {
	output.resize(28);
	Keccak(1152, 448, u8(input.data()), input.size(), 0x06, u8(output.data()), output.size());
}
...
void Sha3_256(const binary &input, binary &output) {
	output.resize(32);
	Keccak(1088, 512, u8(input.data()), input.size(), 0x06, u8(output.data()), output.size());
}
...
void Sha3_384(const binary &input, binary &output) {
	output.resize(48);
	Keccak(832, 768, u8(input.data()), input.size(), 0x06, u8(output.data()), output.size());
}
...
void Sha3_512(const binary &input, binary &output) {
	output.resize(64);
	Keccak(576, 1024, u8(input.data()), input.size(), 0x06, u8(output.data()), output.size());
}
