...
bool Hash::rsaVerify(const binary &digest, const struct rsa_public_key *key, const mpz_t signature)
{
...
void Sha1::process(const binary &data)
{
	sha1_update(&mCtx, data.size(), u8(data.data()));
}
...
void Sha1::pbkdf2(const binary &secret, const binary &salt, binary &key, size_t key_len, unsigned iterations)
{
	Assert(iterations > 0);
	key.resize(key_len);
	pbkdf2_hmac_sha1(secret.size(), u8(secret.data()),
		iterations,
		salt.size(), u8(salt.data()),
		key.size(), u8(key.data()));
}

bool Sha1::rsaVerify(const binary &digest, const struct rsa_public_key *key, const mpz_t signature)
{
...
void Sha256::process(const binary &data)
{
	sha256_update(&mCtx, data.size(), u8(data.data()));
}
...
void Sha256::pbkdf2(const binary &secret, const binary &salt, binary &key, size_t key_len, unsigned iterations)
{
	Assert(iterations > 0);
	key.resize(key_len);
	pbkdf2_hmac_sha256(secret.size(), u8(secret.data()),
		iterations,
		salt.size(), u8(salt.data()),
		key.size(), u8(key.data()));
}

bool Sha256::rsaVerify(const binary &digest, const struct rsa_public_key *key, const mpz_t signature)
{
	if(digest.size() != length()) return false;
...
void Sha512::process(const binary &data)
{
	sha512_update(&mCtx, data.size(), u8(data.data()));
}
...
void Sha512::pbkdf2(const binary &secret, const binary &salt, binary &key, size_t key_len, unsigned iterations)
{
	Assert(iterations != 0);
...
bool Sha512::rsaVerify(const binary &digest, const struct rsa_public_key *key, const mpz_t signature)
{
	if(digest.size() != length()) return false;
...
void Sha3_256::process(const binary &data)
{
	sha3_256_update(&mCtx, data.size(), u8(data.data()));
}
...
void Sha3_512::process(const binary &data)
{
	sha3_512_update(&mCtx, data.size(), u8(data.data()));
}
...
Rsa::PublicKey::PublicKey(const Rsa::PublicKey &key) : PublicKey()
{
	*this = key;
}
...
Rsa::PublicKey::PublicKey(gnutls_x509_crt_t crt)
{
	...
	catch(const std::exception &e)
	{
		...
		throw std::runtime_error("Unable to get RSA public key from x509 certificate: " + string(e.what()));
	}
}
...
void Rsa::PublicKey::fromString(string str)
{
	fromBinary(from_base64(str));
}
...
void Rsa::PrivateKey::sign(Hash &hash, const binary &digest, binary &signature) const
{
	...
	try {
		...
		if(!success) throw std::runtime_error("RSA signature failed");
...
string Rsa::PrivateKey::toString() const
{
	return to_base64(toBinary());
}

void Rsa::PrivateKey::fromString(string str)
{
	fromBinary(from_base64(str));
}
...
void Rsa::generate(PrivateKey &priv)
{
	...
	if(!rsa_generate_keypair (&priv.mPub, &priv.mPriv, NULL, Random::wrapperKey, NULL, NULL, mBits, 0 /*e already set*/))
		throw std::runtime_error("RSA keypair generation failed (size=" + to_string(mBits) + ")");
}

void Rsa::CreateCertificate(gnutls_x509_crt_t crt, gnutls_x509_privkey_t key, const PrivateKey &priv, const string &name)
{
	if(priv.isNull())
		throw std::runtime_error("Creating certificate from null key pair");

	binary b_n; mpz_export_binary(priv.mPub.n,  b_n);
	binary b_e; mpz_export_binary(priv.mPub.e,  b_e);
	binary b_d; mpz_export_binary(priv.mPriv.d, b_d);
	binary b_p; mpz_export_binary(priv.mPriv.p, b_p);
	binary b_q; mpz_export_binary(priv.mPriv.q, b_q);
	binary b_c; mpz_export_binary(priv.mPriv.c, b_c);

	gnutls_datum_t n; n.data = u8(b_n.data()); n.size = b_n.size();
	gnutls_datum_t e; e.data = u8(b_e.data()); e.size = b_e.size();
	gnutls_datum_t d; d.data = u8(b_d.data()); d.size = b_d.size();
	gnutls_datum_t p; p.data = u8(b_p.data()); p.size = b_p.size();
	gnutls_datum_t q; q.data = u8(b_q.data()); q.size = b_q.size();
	gnutls_datum_t c; c.data = u8(b_c.data()); c.size = b_c.size();

	int ret = gnutls_x509_privkey_import_rsa_raw(key, &n, &e, &d, &p, &q, &c);
	if(ret != GNUTLS_E_SUCCESS)
		throw std::runtime_error("Unable to convert RSA key pair to X509: " + string(gnutls_strerror(ret)));

	using clock = std::chrono::system_clock;
	gnutls_x509_crt_set_activation_time(crt, clock::to_time_t(clock::now() - std::chrono::hours(1)));
	gnutls_x509_crt_set_expiration_time(crt, clock::to_time_t(clock::now() + std::chrono::hours(24*365)));
...
void Rsa::SignCertificate(gnutls_x509_crt_t crt, gnutls_x509_crt_t issuer, gnutls_x509_privkey_t issuerKey)
{
	...
	if(ret != GNUTLS_E_SUCCESS)
		throw std::runtime_error("Unable to sign X509 certificate: " + string(gnutls_strerror(ret)));
}
...
bool DerSequence::readLength(BinaryFormatter &formatter, size_t &length)
{
	...
	switch(len)
	{
		...
		{
			uint8_t tmp = 0;
			if(!(formatter >> tmp)) throw std::invalid_argument("Truncated DER size");
			length = size_t(tmp);
			break;
		}
		case 2:
