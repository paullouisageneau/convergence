...
inline Scheduler::Scheduler(size_t threads) : ThreadPool(threads) {
...
auto Scheduler::schedule(task_id &id, time_point time, F &&f, Args &&... args)
...

	{
		std::unique_lock<std::mutex> lock(mutex);
		if (joining)
			throw std::runtime_error("schedule on closing Scheduler");
...
inline void Scheduler::wait(Scheduler::task_id id) {
	if (id.second) {
		...
		pendingCondition.wait(lock, [this, id]() { return pending.find(id) == pending.end(); });
	}
}
