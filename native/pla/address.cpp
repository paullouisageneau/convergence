...
bool Address::Resolve(const string &host, const string &service, std::list<Address> &result) {
	...
	if (host.find(':') != string::npos || !hasalpha(host))
...
bool Address::Reverse(const Address &a, string &result) {
	...
	if (getnameinfo(a.addr(), a.addrLen(), host, HOST_NAME_MAX, service, SERVICE_NAME_MAX,
	                NI_NUMERICSERV)) {
		result = a.toString();
		return false;
	}
...
Address::Address(void) { clear(); }

Address::Address(const string &host, const string &service) { set(host, service); }

Address::Address(const string &host, uint16_t port) { set(host, port); }

Address::Address(const string &str) { set(str); }

Address::Address(const sockaddr *addr, socklen_t addrlen) { set(addr, addrlen); }
...
void Address::set(const string &str) { fromString(str); }
...
bool Address::isEmpty(void) const { return (mAddrLen == 0); }
...
bool Address::isPrivate(void) const {
	...
	case AF_INET6: // IPv6
	{
		...
		if (b[10] == 0xFF && b[11] == 0xFF) // mapped
		{
			if (b[12] == 10)
				return true;
			if (b[12] == 172 && b[13] >= 16 && b[13] < 32)
				return true;
			if (b[12] == 192 && b[13] == 168)
				return true;
		}
		break;
	}
...
bool Address::isPublic(void) const { return !isLocal() && !isPrivate(); }

bool Address::isIpv4(void) const { return (addrFamily() == AF_INET); }

bool Address::isIpv6(void) const { return (addrFamily() == AF_INET6); }
...
string Address::host(bool numeric) const {
	...
	char host[HOST_NAME_MAX];
	if (getnameinfo(addr(), addrLen(), host, HOST_NAME_MAX, NULL, 0,
...
string Address::service(bool numeric) const {
	...
	char service[SERVICE_NAME_MAX];
	if (getnameinfo(addr(), addrLen(), NULL, 0, service, SERVICE_NAME_MAX,
...
string Address::reverse(bool numericHost) const {
	...
	char host[HOST_NAME_MAX];
	char service[SERVICE_NAME_MAX];
	if (getnameinfo(addr(), addrLen(), host, HOST_NAME_MAX, service, SERVICE_NAME_MAX,
	                (numericHost ? NI_NUMERICHOST : 0) | NI_NUMERICSERV))
		throw std::invalid_argument("Invalid stored network address");
...
const sockaddr *Address::addr(void) const { return reinterpret_cast<const sockaddr *>(&mAddr); }

int Address::addrFamily(void) const {
	return reinterpret_cast<const sockaddr_in *>(&mAddr)->sin_family;
}

socklen_t Address::addrLen(void) const { return mAddrLen; }

bool operator<(const Address &a1, const Address &a2) {
	if (a1.addrLen() != a2.addrLen())
		return a1.addrLen() < a2.addrLen();
	if (a1.addrFamily() != a2.addrFamily())
		return a1.addrFamily() < a2.addrFamily();

	if (!a2.isLocal()) {
		if (a1.isLocal())
			return true;
		if (a1.isPrivate() && !a2.isPrivate())
			return true;
	}
	if (!a1.isLocal()) {
		if (a2.isLocal())
			return false;
		if (a2.isPrivate() && !a1.isPrivate())
			return false;
	}
	...
	case AF_INET: // IPv4
	{
		...
		return (sa1->sin_addr.s_addr < sa2->sin_addr.s_addr ||
		        (sa1->sin_addr.s_addr == sa2->sin_addr.s_addr && sa1->sin_port < sa2->sin_port &&
	...
	case AF_INET6: // IPv6
	{
		...
		return (cmp < 0 ||
		        (cmp == 0 && sa1->sin6_port < sa2->sin6_port && sa1->sin6_port && sa2->sin6_port));
	}
...
bool operator==(const Address &a1, const Address &a2) {
	...
	case AF_INET: // IPv4
	{
		...
		const sockaddr_in *sa2 = reinterpret_cast<const sockaddr_in *>(a2.addr());
		return (sa1->sin_addr.s_addr == sa2->sin_addr.s_addr &&
		        (sa1->sin_port == sa2->sin_port || !sa1->sin_port || !sa2->sin_port));
	}
	...
	case AF_INET6: // IPv6
	{
		const sockaddr_in6 *sa1 = reinterpret_cast<const sockaddr_in6 *>(a1.addr());
		const sockaddr_in6 *sa2 = reinterpret_cast<const sockaddr_in6 *>(a2.addr());
		return (std::memcmp(sa1->sin6_addr.s6_addr, sa2->sin6_addr.s6_addr, 16) == 0 &&
		        (sa1->sin6_port == sa2->sin6_port || !sa1->sin6_port || !sa2->sin6_port));
	}
