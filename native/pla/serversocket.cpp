...
ServerSocket::ServerSocket(void) : mSock(INVALID_SOCKET), mPort(0) {}

ServerSocket::ServerSocket(int port) : mSock(INVALID_SOCKET), mPort(0) { listen(port); }

ServerSocket::~ServerSocket(void) { NOEXCEPTION(close()); }
...
bool ServerSocket::isClosed(void) const { return (mSock == INVALID_SOCKET); }
