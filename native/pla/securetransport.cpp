	...
	mIsByeDone(false)
{
	...
	if(stream->isMessage()) flags|= GNUTLS_DATAGRAM;
...
void SecureTransport::handshake(void)
{
	...
	check_gnutls(gnutls_priority_set_direct(mSession, mPriorities.c_str(), &err_pos), "Unable to set TLS priorities: " + mPriorities);
...
bool SecureTransport::isMessage(void) const
{
	return mStream->isMessage();
}
...
int SecureTransport::TimeoutCallback(gnutls_transport_ptr_t ptr, unsigned int ms)
{
	...
	try {
		gnutls_transport_set_errno(st->mSession, 0);
		if(st->mStream->wait(milliseconds(ms))) return 1;
		else return 0;
	}
	catch(const std::exception &e)
	{
...
int SecureTransport::CertificateCallback(gnutls_session_t session)
{
	...
	try {
		if(!transport->mVerifier)
		{
			...
			{
				string reasons;
				if(status & GNUTLS_CERT_SIGNER_NOT_FOUND)	reasons+= "unknown issuer,";
				else if(status & GNUTLS_CERT_REVOKED)		reasons+= "revoked,";
				else if(status & GNUTLS_CERT_EXPIRED)		reasons+= "expired,";
				else if(status & GNUTLS_CERT_NOT_ACTIVATED)	reasons+= "not yet activated,";
				else if(status & GNUTLS_CERT_INVALID)		reasons+= "not trusted,";
				reasons.pop_back();
		...
		for(int i=0; i<count; ++i)
		{
			...
			try {
				...
				if(ret != GNUTLS_E_SUCCESS) throw std::runtime_error(string("Unable to retrieve X509 certificate: ") + Errorstring(ret));
	...
	Certificate()
{
	...
	check_gnutls(gnutls_certificate_set_x509_key_file2(mCreds,
...
SecureTransport::RsaCertificate::RsaCertificate(const Rsa::PrivateKey &priv, const string &name, const SecureTransport::RsaCertificate *issuer)
{
	...
	try {
		...
		if(ret != GNUTLS_E_SUCCESS)
			throw std::runtime_error(string("Unable to set certificate and key pair in credentials: ") + Errorstring(ret));
	}
	catch(...)
	{
...
SecureTransportClient::PrivateSharedKey::PrivateSharedKey(const string &username, const binary &key)
{
	// Allocate PSK credentials
	check_gnutls(gnutls_psk_allocate_client_credentials(&mCreds));
	...
	try {
		check_gnutls(gnutls_psk_set_client_credentials(mCreds, username.c_str(), &datum, GNUTLS_PSK_KEY_RAW), "Unable to set credentials");
	}
	catch(...)
	{
...
int SecureTransportServer::PostClientHelloCallback(gnutls_session_t session)
{
	...
	SecureTransportServer *transport = static_cast<SecureTransportServer*>(gnutls_session_get_ptr(session));
	if(!transport)
	{
