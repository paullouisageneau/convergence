...
Socket::Socket(void)
    : mSock(INVALID_SOCKET), mConnectTimeout(seconds(-1.)), mReadTimeout(seconds(-1.)),
...
Socket::Socket(socket_t sock)
    : mConnectTimeout(seconds(-1.)), mReadTimeout(seconds(-1.)), mWriteTimeout(seconds(-1.)) {
	mSock = sock;
}

Socket::~Socket(void) { NOEXCEPTION(close()); }

bool Socket::isConnected(void) const { return (mSock != INVALID_SOCKET); }
...
void Socket::setConnectTimeout(duration timeout) { mConnectTimeout = timeout; }

void Socket::setReadTimeout(duration timeout) { mReadTimeout = timeout; }

void Socket::setWriteTimeout(duration timeout) { mWriteTimeout = timeout; }
