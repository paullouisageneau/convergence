...
Mesh::Mesh(const index_t *indices, size_t nindices, const float *vertices, size_t nvertices)
    : Mesh() {
...
float Mesh::getRadius(void) const { return mRadius; }
...
float Mesh::intersect(const vec3 &pos, const vec3 &move, float radius, vec3 *intersection) {
	...
	if (mRadius >= 0.f && glm::length(pos) > mRadius &&
