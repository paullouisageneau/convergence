...
float intersectBox(const vec3 &p, const vec3 &move, float radius, const vec3 &min, const vec3 &max,
	...
	t[0] = intersectFace(p, move, radius, vec3(min.x, min.y, min.z), vec3(max.x, min.y, min.z),
	                     vec3(max.x, max.y, min.z), vec3(min.x, max.y, min.z), &inter[0]);
	t[1] = intersectFace(p, move, radius, vec3(min.x, min.y, max.z), vec3(max.x, min.y, max.z),
	                     vec3(max.x, max.y, max.z), vec3(min.x, max.y, max.z), &inter[1]);
	t[2] = intersectFace(p, move, radius, vec3(min.x, min.y, min.z), vec3(min.x, max.y, max.z),
	                     vec3(min.x, max.y, max.z), vec3(min.x, min.y, max.z), &inter[2]);
	t[3] = intersectFace(p, move, radius, vec3(max.x, min.y, min.z), vec3(max.x, max.y, min.z),
	                     vec3(max.x, max.y, max.z), vec3(max.x, min.y, max.z), &inter[3]);
	t[4] = intersectFace(p, move, radius, vec3(min.x, min.y, min.z), vec3(max.x, min.y, min.z),
	                     vec3(max.x, min.y, max.z), vec3(min.x, min.y, max.z), &inter[4]);
	t[5] = intersectFace(p, move, radius, vec3(min.x, max.y, min.z), vec3(max.x, max.y, min.z),
	                     vec3(max.x, max.y, max.z), vec3(min.x, max.y, max.z), &inter[5]);
...
float intersectFace(const vec3 &p, const vec3 &move, float radius, const vec3 &p1, const vec3 &p2,
	...
	float planeDist =
	    intersectPlane(p, -normal, p1, normal); // distance de la source au plan de la face
	if (planeDist < 0.f)
		return std::numeric_limits<float>::infinity();
	...
	if (isInTriangle(planeintersection, p1, p2, p3))
		polygonintersection = planeintersection;
	else
		polygonintersection = closestPointOnTriangle(planeintersection, p1, p2, p3);

	if (intersection)
		*intersection = polygonintersection;

	// si on est déjà en train de passer à travers un polygone
	if (glm::distance(p, polygonintersection) < radius)
		return 0.f;
